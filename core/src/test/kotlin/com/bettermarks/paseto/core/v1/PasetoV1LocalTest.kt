...
class PasetoV1LocalTest {
  ...
  fun encrypt(
    ...
    if (expectFail) {
      assertNotEquals(token, PasetoV1Local.encrypt(secretKey, nonce, payload ?: "", footer))
    } else {
      assertEquals(token, PasetoV1Local.encrypt(secretKey, nonce, payload ?: "", footer))
    }
  }
  ...
  fun decrypt(
    ...
    if (expectFail) {
      assertThrows<PasetoException> { PasetoV1Local.decrypt(secretKey, parsedToken) }
    } else {
      val (actualMessage, actualFooter) = PasetoV1Local.decrypt(secretKey, parsedToken)
  ...
  companion object {
    ...
    fun testVectors(): List<Arguments> =
        TestVectors.read(Version.V1, Purpose.Local).map { vector ->
          Arguments.of(
              vector.name,
              vector.expectFail,
              vector.key(Version.V1),
              vector.nonce,
              vector.payload,
              vector.footer,
              vector.token)
